...
contract ProcessReferralCSV is Script {
    ...
    function parseCSV(string memory csvPath) internal view returns (ReferralData[] memory) {
        ...
        if (bytes(lines[0]).length > 0) {
            ...
            if (keccak256(bytes(vm.toLowercase(lines[0]))) == keccak256(bytes("address,amount")) ||
        ...
        for (uint256 i = startIndex; i < lines.length; i++) {
            ...
            referrals[i - startIndex] = ReferralData({
                user: user,
                amount: amount
            });
        }
    ...
    function validateReferralData(ReferralData[] memory referrals) internal pure {
        ...
        for (uint256 i = 0; i < referrals.length; i++) {
            ...
            require(
                ...
                string(abi.encodePacked(
                    "User ", vm.toString(referrals[i].user),
                    " exceeds max reward: ", vm.toString(referrals[i].amount)
                ))
            );
            ...
            for (uint256 j = i + 1; j < referrals.length; j++) {
                require(
                    ...
                    string(abi.encodePacked(
                        "Duplicate address: ", vm.toString(referrals[i].user)
                    ))
                );
            }
        ...
        require(
            ...
            string(abi.encodePacked(
                "Total allocation exceeds cap: ", vm.toString(totalAllocation)
            ))
        );
