...
contract VerifyAddresses is Script {
     ...
     */
    function readDeployment(string memory chainDir, bool isTestnet) internal view returns (ChainDeployment memory deployment) {
     ...
     */
    function readFromJson(string memory basePath, ChainDeployment memory deployment) internal view returns (bool success, ChainDeployment memory) {
     ...
     */
    function readFromTextFiles(string memory basePath, ChainDeployment memory deployment) internal view returns (ChainDeployment memory) {
        ...
        try vm.readFile(string.concat(projectRoot, "/", basePath, "HolographFactoryProxy.txt")) returns (string memory addr) {
    ...
    function checkContract(
        ChainDeployment[] memory deployments,
        address refAddress,
        ...
        for (uint256 i = 0; i < deployments.length; i++) {
            ...
            if (addr != address(0)) {
                ...
                if (addr != refAddress && refAddress != address(0)) {
                    ...
                    console.log("  [ERROR] %s: %s (expected: %s)", 
                        deployments[i].chainName, 
                        addr, 
                        refAddress
                    );
                    issues++;
