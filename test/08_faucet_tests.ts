...
import { Faucet, SampleERC20 } from '../typechain-types'
...
describe('Testing the Holograph Faucet', async () => {
  ...
  before(async function () {
    l1 = await setup()
    SAMPLEERC20 = l1.sampleErc20.attach(l1.sampleErc20Holographer.address)
    FAUCET = l1.faucet
    ...
    SAMPLEERC20.functions.mint(FAUCET.address, INITIAL_FAUCET_FUNDS)
  })
  ...
  describe('Test Initializer', async function () {
    ...
    it('should fail initializing already initialized Faucet', async function () {
      await expect(
        FAUCET.init(generateInitCode(['address', 'address'], [DEPLOYER, SAMPLEERC20.address]))
  ...
  describe('Default drip flow', async () => {
    ...
    it('isAllowedToWithdraw(): User is allowed to withdraw for the first time', async function () {
      expect(
        await FAUCET.functions.isAllowedToWithdraw(DEPLOYER)
      ).to.be.true
    })
    ...
    it('requestTokens(): User can withdraw for the first time', async function () {
      ...
      dripCount++
      expect(
        await SAMPLEERC20.functions.balanceOf(DEPLOYER)
      ).to.equal(DEFAULT_DRIP_AMOUNT)
    })
    ...
    it('requestTokens(): User cannot withdraw for the second time', async function () {
      expect(
        await FAUCET.functions.requestTokens()
      ).to.be.revertedWith(COME_BACK_LATER)
    })
  ...
  describe('Owner drip flow', async () => {
    ...
    it('grantTokens(): Owner can grant tokens', async function () {
      ...
      expect(
        await SAMPLEERC20.functions.balanceOf(DEPLOYER)
      ).to.equal(DEFAULT_DRIP_AMOUNT.mul(2))
    })

    it('grantTokens(): Owner can grant tokens again with arbitrary amount', async function () {
      const factor = 2
      await FAUCET.functions['grantTokens(address,uint256)'](DEPLOYER, DEFAULT_DRIP_AMOUNT.mul(factor)).then(tx => tx.wait())
      dripCount += factor
      expect(
        await SAMPLEERC20.functions.balanceOf(DEPLOYER)
      ).to.equal(DEFAULT_DRIP_AMOUNT.mul(2 + factor))
    })
  ...
  describe('Owner can adjust Withdraw Cooldown', async () => {
    ...
    it('setWithdrawCooldown(): Owner adjusts Withdraw Cooldown to 0 seconds', async function () {
      expect(
        await FAUCET.functions.setWithdrawCooldown(0)
      ).to.not.reverted
    ...
    it('setWithdrawCooldown(): Owner adjusts Withdraw Cooldown back to 24 hours', async function () {
      expect(
        await FAUCET.functions.setWithdrawCooldown(DEFAULT_DRIP_COOLDOWN)
      ).to.not.reverted
    ...
    it(`setWithdrawCooldown(): User can't adjust Withdraw Cooldown`, async function () {
      expect(
        await FAUCET.connect(USER_A).functions.setWithdrawCooldown(0)
      ).to.revertedWith(NOT_AN_OWNER)
    })
  ...
  describe('Owner can adjust Withdraw Amount', async () => {
    ...
    it('setWithdrawAmount(): Owner adjusts Withdraw Amount', async function () {
      expect(
        await FAUCET.functions.setWithdrawAmount(DEFAULT_DRIP_AMOUNT.mul(factor))
      ).to.not.reverted
    ...
    it('requestTokens(): User can withdraw increased amount', async function () {
      await FAUCET.connect(USER_A).functions.requestTokens().then(tx => tx.wait())
      dripCount += factor
      expect(
        await SAMPLEERC20.functions.balanceOf(USER_A)
      ).to.equal(DEFAULT_DRIP_AMOUNT.mul(factor))
    })

    it('setWithdrawAmount(): Owner adjusts Withdraw Amount back to 100 eth', async function () {
      expect(
        await FAUCET.functions.setWithdrawAmount(DEFAULT_DRIP_AMOUNT)
      ).to.not.reverted
    ...
    it(`setWithdrawAmount(): User can't adjust Withdraw Amount`, async function () {
      expect(
        await FAUCET.connect(USER_A).functions.setWithdrawAmount(0)
      ).to.revertedWith(NOT_AN_OWNER)
    })
  ...
  describe('Owner can Withdraw Faucet funds', async () => {
    ...
    it('withdrawTokens(address,uint256)', async function () {
      ...
      dripCount++
      expect(
        await SAMPLEERC20.functions.balanceOf(USER_B)
      ).to.equal(DEFAULT_DRIP_AMOUNT)
    })
    ...
    it('withdrawTokens(address)', async function () {
      ...
      expect(
        await SAMPLEERC20.functions.balanceOf(USER_C)
      ).to.equal(INITIAL_FAUCET_FUNDS.sub(DEFAULT_DRIP_AMOUNT.mul(dripCount)))
      expect(
        await SAMPLEERC20.functions.balanceOf(FAUCET.address)
      ).to.equal(0)
    })
