...
contract HolographOperator is Admin, Initializable, HolographOperatorInterface {
  ...
  /**
   * @dev Deletes a job from the _deprecatedOperatorJobs mapping.
   * @param jobHash The hash of the job to delete.
   */
  function deleteOperatorJob(bytes32 jobHash) external onlyAdmin {
    if (_deprecatedOperatorJobs[jobHash] == 0) {
      revert JobDoesNotExist(jobHash);
    }
    delete _deprecatedOperatorJobs[jobHash];
  }

  /**
   * @dev Deletes a job from the _deprecatedFailedJobs mapping.
   * @param jobHash The hash of the job to delete.
   */
  function deleteFailedJob(bytes32 jobHash) external onlyAdmin {
    if (!_deprecatedFailedJobs[jobHash]) {
      revert JobDoesNotExist(jobHash);
    }
    delete _deprecatedFailedJobs[jobHash];
  }

  /**
   * @dev Deletes multiple jobs from the _deprecatedOperatorJobs mapping.
  ...
  function deleteMultipleOperatorJobs(bytes32[] memory jobHashes) external onlyAdmin {
    for (uint256 i = 0; i < jobHashes.length; i++) {
      if (_deprecatedOperatorJobs[jobHashes[i]] == 0) {
        revert JobDoesNotExist(jobHashes[i]);
      }
      delete _deprecatedOperatorJobs[jobHashes[i]];
    }
  }

  /**
   * @dev Deletes multiple jobs from the _deprecatedFailedJobs mapping.
   * @param jobHashes An array of job hashes to delete.
   */
  function deleteMultipleFailedJobs(bytes32[] memory jobHashes) external onlyAdmin {
    for (uint256 i = 0; i < jobHashes.length; i++) {
      if (!_deprecatedFailedJobs[jobHashes[i]]) {
        revert JobDoesNotExist(jobHashes[i]);
      }
      delete _deprecatedFailedJobs[jobHashes[i]];
    }
  }
  ...
  function failedJobExists(bytes32 jobHash) external view returns (bool) {
    return _deprecatedFailedJobs[jobHash];
  }
